...
common() {
  echo -n "\"border\": \"$bg_bar_color\","
  echo -n "\"separator\":true,"
  echo -n "\"separator_block_width\":5,"
  echo -n "\"border_top\":2,"
  echo -n "\"border_bottom\":2,"
  echo -n "\"border_left\":0,"
  echo -n "\"border_right\":0"
...
mycrypto() {
  ...
  echo -n "\"full_text\":\" $(~/.config/i3status/i3status_py/crypto.py) \","
  echo -n "\"color\":\"#000000\","
...
myip_public() {
  ...
  echo -n "\"full_text\":\" $(~/.config/i3status/i3status_py/ip.py) \","
  echo -n "\"color\":\"#000000\","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "},"
...
disk_usage() {
  ...
  echo -n "\"full_text\":\"  $(~/.config/i3status/i3status_py/disk.py)% \","
  echo -n "\"color\":\"#000000\","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "}"
...
memory() {
  ...
  echo -n "\"full_text\":\"   $(~/.config/i3status/i3status_py/memory.py)% \","
  echo -n "\"color\":\"#000000\","
  echo -n "\"background\":\"#9DD068\","
  common
  echo -n "}"
}
...
cpu_usage() {
  ...
  echo -n "\"full_text\":\"  $(~/.config/i3status/i3status_py/cpu.py)% \","
  echo -n "\"color\":\"#000000\","
  echo -n "\"background\":\"#EF8E35\","
  common
  echo -n "},"
}
...
meteo() {
  ...
  echo -n "\"full_text\":\" $(~/.config/i3status/i3status_py/meteo.py) \","
  echo -n "\"color\":\"#000000\","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "},"
...
do
  ...
  if [[ $line == *"name"*"id_vpn"* ]]; then
    alacritty -e ~/.config/i3status/i3status_py/click_vpn.sh &

  # CHECK UPDATES
  elif [[ $line == *"name"*"id_systemupdate"* ]]; then
    alacritty -e ~/.config/i3status/i3status_py/click_checkupdates.sh &
  ...
  # TIME
    ...
    alacritty -e ~/.config/i3status/i3status_py/click_time.sh &

  # METEO
  elif [[ $line == *"name"*"id_meteo"* ]]; then
    xdg-open https://openweathermap.org/city/554840 > /dev/null &
